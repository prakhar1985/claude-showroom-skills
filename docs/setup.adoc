...
[IMPORTANT]
====
*This section is for advanced users who already have `~/.claude/` with:*

* Custom `settings.json` or other configuration files
* Other Claude Code skills installed
* Custom prompts or hooks
* Any existing Claude Code setup they want to preserve

If you're a new user with no existing `~/.claude/` directory, use the link:#installation-steps[Standard Installation] above instead.
====

=== Merge Showroom Skills Without Overwriting

Follow these steps to add Showroom skills alongside your existing configuration:
