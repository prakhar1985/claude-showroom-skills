...
2. *Skills auto-load* from the project's `.cursor/rules/` directory
3. *Use natural language* in Cursor's chat:
   - "create lab" or "generate workshop module"
   - "create demo" or "presenter-led demo"
   - "verify content" or "check quality"

[IMPORTANT]
====
Cursor uses **natural language triggers**, NOT slash commands. Say "create lab" instead of `/create-lab`.
====

Detailed Cursor setup: See link:https://github.com/rhpds/showroom_template_nookbag/blob/main/CURSOR-SKILLS-README.adoc[CURSOR-SKILLS-README.adoc^] in the nookbag template.
