...
git clone git@github.com:rhpds/showroom_template_nookbag.git
...
=== Conflicts with Existing ~/.claude/ Configuration

*Problem:* Already have `~/.claude/` with custom settings, other skills, or configurations

*Symptoms:*

* Don't want to overwrite existing `settings.json`
* Have other skills installed that might conflict
* Have custom prompts or hooks

*Solution:* Use selective installation (Option C from setup guide)
...
# 2. Copy only Showroom skills (one by one to avoid overwriting)
...
# 3. Copy prompts (required, but won't conflict with custom prompts)
mkdir -p ~/.claude/prompts
cp .claude/prompts/* ~/.claude/prompts/

# 4. Verify both old and new skills work
claude
/help  # Should list all skills (existing + Showroom)
----
...
* *settings.json*: Showroom skills don't require special settings - your existing settings.json is fine
* *Other skills*: Multiple skills coexist peacefully - no conflicts
* *Prompts*: Showroom prompts are uniquely named (won't overwrite custom prompts)
* *Hooks*: If you have custom hooks, review `.claude/hooks/` before copying
...
For common AsciiDoc formatting fixes (external links, lists, images, em dashes, product names), see:

* link:reference.html#common-validation-fixes[Quick Reference → Common Validation Fixes]

This section includes before/after examples for all common issues reported by `/verify-content`.
...
==== 1. Skills not in home directory
...
[source,bash]
----
# Ensure skills are in home directory (global)
cp -r /path/to/showroom_template_nookbag/.claude ~/.claude
----

[IMPORTANT]
====
For Showroom skills, *always use `~/.claude/` (home directory)*, not project-specific `.claude/`.

Cursor reads from both locations, but home directory ensures:

* Skills work across all projects
* No need to copy skills to every project
* Consistent behavior with Claude Code CLI
====
...
* link:https://github.com/rhpds/showroom_template_nookbag/issues[GitHub Issues - showroom_template_nookbag^]
